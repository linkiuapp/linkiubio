...
/**
 * Fiscal Validation Service Unit Tests
 * 
 * Tests for fiscal information validation without database dependencies
 * Requirements: 3.3, 3.4 - Business document validation and compliance system
 */
class FiscalValidationServiceTest extends TestCase
{
    ...
    public function test_validates_colombian_nit_correctly()
    {
        // Valid NIT format
    ...
    public function test_validates_chilean_rut_correctly()
    {
        // Valid RUT format
    ...
    public function test_validates_mexican_rfc_correctly()
    {
        // Valid RFC format - use a more realistic RFC
        $result = $this->fiscalService->validateFiscalDocument('rfc', 'XAXX010101000', 'México');
        $this->assertTrue($result['isValid'], 'RFC validation failed: ' . ($result['message'] ?? 'No message'));
    ...
    public function test_validates_cedula_correctly()
    {
        // Valid cedula
    ...
    public function test_provides_document_suggestions()
    {
    ...
    public function test_returns_tax_regimes_by_country()
    {
        // Colombia
    ...
    public function test_validates_tax_regime_for_country()
    {
    ...
    public function test_validates_complete_fiscal_information()
    {
        $validFiscalData = [
    ...
    public function test_validates_incomplete_fiscal_information()
    {
    ...
    public function test_requires_legal_representative_declaration_for_companies()
    {
        $companyFiscalData = [
            'fiscal_document_type' => 'nit',
            'fiscal_document_number' => '900123456-7',
    ...
    public function test_does_not_require_legal_representative_declaration_for_individuals()
    {
    ...
    public function test_gets_document_types_by_country()
    {
        // Colombia
    ...
    public function test_validates_nit_with_verification_digit()
    {
    ...
    public function test_validates_rut_with_verification_digit()
    {
        // Test RUT format validation
        $result = $this->fiscalService->validateFiscalDocument('rut', '12345678-9', 'Chile');
        $this->assertTrue($result['isValid']);

        // Test invalid RUT format
        $result = $this->fiscalService->validateFiscalDocument('rut', '12345678', 'Chile');
        $this->assertFalse($result['isValid']);
    }

    public function test_handles_empty_document_validation()
    {
        $result = $this->fiscalService->validateFiscalDocument('', '', '');
        $this->assertFalse($result['isValid']);
        $this->assertEquals('Tipo y número de documento son requeridos', $result['message']);
    }

    public function test_handles_invalid_document_type()
    {
        $result = $this->fiscalService->validateFiscalDocument('invalid_type', '123456789', 'Colombia');
        $this->assertFalse($result['isValid']);
        $this->assertEquals('Tipo de documento no válido', $result['message']);
    }

    public function test_validates_address_length()
    {
        $fiscalData = [
            'fiscal_document_type' => 'cedula',
            'fiscal_document_number' => '12345678',
            ...
            'fiscal_department' => 'Cundinamarca',
            'fiscal_city' => 'Bogotá',
            'fiscal_address' => 'Short', // Too short
            'tax_regime' => 'simplificado',
            'tax_responsibility_declaration' => true,
            'fiscal_data_processing_consent' => true
        ];

        $result = $this->fiscalService->validateFiscalInformation($fiscalData);
        $this->assertFalse($result['isValid']);
        $this->assertArrayHasKey('fiscal_address', $result['errors']);
        $this->assertStringContainsString('mínimo 10 caracteres', $result['errors']['fiscal_address']);
    }
}