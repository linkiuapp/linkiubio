...
<?php $__env->startSection('title', 'Editar Pedido #' . $order->order_number); ?>
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-4">
            <a href="<?php echo e(route('tenant.admin.orders.show', [$store->slug, $order->id])); ?>" 
               class="text-gray-600 hover:text-gray-700">
                <i data-lucide="arrow-left" class="w-6 h-6"></i>
            </a>
            <h1 class="text-lg font-semibold text-gray-900">Editar Pedido #<?php echo e($order->order_number); ?></h1>
            <span class="text-xs <?php echo e($order->status_color); ?> px-2 py-1 rounded">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-xs text-yellow-700 mt-1">
                        Este pedido está en estado "<?php echo e($order->status_label); ?>" y solo se pueden editar algunos campos básicos.
                    </div>
                </div>
            </div>
        </div>
    ...
    <form action="<?php echo e(route('tenant.admin.orders.update', [$store->slug, $order->id])); ?>" method="POST" enctype="multipart/form-data" x-data="orderEdit" x-init="init()" @submit="validateForm" class="space-y-6">
        <?php echo csrf_field(); ?>
        <?php echo method_field('PUT'); ?>
        
        <!-- Inputs hidden para asegurar que siempre se envíen valores -->
        <input type="hidden" name="customer_address" :value="deliveryType === 'pickup' ? '' : (document.getElementById('customer_address')?.value || '<?php echo e($order->customer_address ?? ''); ?>')">
                ...
                <template x-if="deliveryType === 'local' || deliveryType === 'national'">
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="customer_address" 
                                  ...
                                  rows="3"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-gray-900 placeholder-gray-400"
                                  placeholder="Dirección completa, barrio, referencias"><?php echo e($addressValue); ?></textarea>
        ...
        <div class="bg-gray-50 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="space-y-3">
                        <?php if($simpleShipping && $simpleShipping->pickup_enabled): ?>
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors">
                            <input type="radio" 
                                   name="delivery_type" 
                                   value="pickup" 
                                   <?php echo e(in_array(old('delivery_type', $order->delivery_type), ['pickup']) ? 'checked' : ''); ?>
                        ...
                        <?php if($simpleShipping && $simpleShipping->local_enabled): ?>
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors">
                            <input type="radio" 
                                   name="delivery_type" 
                                   value="local" 
                                   <?php echo e(in_array(old('delivery_type', $order->delivery_type), ['local', 'domicilio']) ? 'checked' : ''); ?>
                        ...
                        <?php if($simpleShipping && $simpleShipping->national_enabled): ?>
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors">
                            <input type="radio" 
                                   name="delivery_type" 
                                   value="national" 
                                   <?php echo e(in_array(old('delivery_type', $order->delivery_type), ['national']) ? 'checked' : ''); ?>
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors">
                            <input type="radio" 
                                   name="payment_method" 
                                   value="transferencia" 
                                   <?php echo e(old('payment_method', $order->payment_method) === 'transferencia' ? 'checked' : ''); ?>
                        ...
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors">
                            <input type="radio" 
                                   ...
                                   <?php echo e(old('payment_method', $order->payment_method) === 'contra_entrega' ? 'checked' : ''); ?>
                        ...
                        <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors" x-show="deliveryType === 'pickup'">
                            <input type="radio" 
                                   name="payment_method" 
                                   value="efectivo" 
                                   <?php echo e(old('payment_method', $order->payment_method) === 'efectivo' ? 'checked' : ''); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div x-show="paymentMethod === 'transferencia'" class="mt-6">
                    ...
                    <div class="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-3">
                                <a href="<?php echo e(route('tenant.admin.orders.download-payment-proof', [$store->slug, $order->id])); ?>" 
                ...
                <label for="payment_proof" class="block text-sm font-medium text-gray-700 mb-2">
                    <?php echo e($order->payment_proof_path ? 'Reemplazar Comprobante' : 'Comprobante de Pago'); ?> (Opcional)
                </label>
                <div class="w-full">
                    <label class="block">
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="flex justify-end">
                    <div class="w-full max-w-sm space-y-3">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-900">$<?php echo e(number_format($order->subtotal, 0, ',', '.')); ?></span>
                        </div>
                        <?php if($order->shipping_cost > 0): ?>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Envío:</span>
                                <span class="text-gray-900">$<?php echo e(number_format($order->shipping_cost, 0, ',', '.')); ?></span>
                            </div>
                        <?php endif; ?>
                        <?php if($order->coupon_discount > 0): ?>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Descuento:</span>
                                <span class="text-green-600">-$<?php echo e(number_format($order->coupon_discount, 0, ',', '.')); ?></span>
                            </div>
                        <?php endif; ?>
        ...
        <div class="bg-gray-50 rounded-lg p-6">
            ...
            <textarea name="notes" 
                      rows="3"
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-gray-900 placeholder-gray-400"
                      placeholder="Instrucciones especiales, observaciones, etc."><?php echo e(old('notes', $order->notes)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="flex gap-3 justify-end">
            <a href="<?php echo e(route('tenant.admin.orders.show', [$store->slug, $order->id])); ?>" class="px-4 py-2 border-2 border-red-300 text-red-600 rounded-lg hover:bg-red-50 transition-colors flex items-center gap-2">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'design-system::Alerts.AlertBordered','data' => ['type' => 'warning','title' => 'Validación requerida','xMessage' => 'validationMessage']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['type' => 'warning','title' => 'Validación requerida','x-message' => 'validationMessage']); ?>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('orderEdit', () => ({
        ...
        deliveryType: '<?php echo e(old("delivery_type", $order->delivery_type === "domicilio" ? "local" : $order->delivery_type)); ?>',
        paymentMethod: '<?php echo e(old("payment_method", $order->payment_method)); ?>',
        selectedDepartment: '<?php echo e(old("department", $order->department ?? "")); ?>',
        selectedCity: '<?php echo e(old("city", $order->city ?? "")); ?>',
        ...
        loadDepartmentsAndCities() {
            fetch('<?php echo e(route("tenant.admin.orders.shipping-departments", $store->slug)); ?>')
...
<?php endif; ?> <?php /**PATH C:\laragon\www\Liniu_Final\app\Features\TenantAdmin/Views/Core/orders/edit.blade.php ENDPATH**/ ?>