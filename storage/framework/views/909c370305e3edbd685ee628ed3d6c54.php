<?php $__env->startPush('meta'); ?>
    <meta name="description" content="Finaliza tu compra en <?php echo e($store->name); ?> - Proceso de checkout seguro y rápido">
...
function enableStep2Button() {
    ...
    if (shippingType === 'local') {
        ...
        if (isValid) {
            calculateShippingCost(null, '<?php echo e($store->city ?? ""); ?>');
        }
    ...
    items.forEach((item, index) => {
        ...
        html += `
            <div class="flex items-center gap-3 py-2 border-b border-brandWhite-300 last:border-b-0">
                <img src="${item.image_url || (item.product && item.product.main_image_url) || '<?php echo e(asset("assets/images/placeholder-product.svg")); ?>'}" 
...
async function loadShippingMethods() {
    try {
        const response = await fetch('<?php echo e(route("tenant.checkout.shipping-methods", $store->slug)); ?>');
...
async function loadShippingMethods() {
    try {
        ...
        const shippingData = <?php echo json_encode($shippingMethods ?? [], 15, 512) ?>;
...
async function loadPaymentMethods() {
    const url = '<?php echo e(route("tenant.checkout.payment-methods", $store->slug)); ?>';
...
async function loadAvailableTables() {
    const url = '<?php echo e(route("tenant.checkout.available-tables", $store->slug)); ?>';
...
async function loadAvailableRooms() {
    const url = '<?php echo e(route("tenant.checkout.available-rooms", $store->slug)); ?>';
...
async function submitOrder() {
    ...
    try {
        ...
        if (deliveryType) {
            ...
            if (deliveryType.value === 'domicilio') {
                ...
                if (shippingType === 'local') {
                    ...
                    formData.append('department', '<?php echo e($store->department ?? ""); ?>');
                    formData.append('city', '<?php echo e($store->city ?? ""); ?>');
        ...
        // Enviar al servidor
        const checkoutUrl = '<?php echo e(route("tenant.checkout.store", $store->slug)); ?>';
        ...
        if (data.success) {
            ...
            window.location.href = `<?php echo e(route("tenant.checkout.success", $store->slug)); ?>?order=${data.order.id}`;
        } else {
            ...
            if (data.error === 'stock_unavailable') {
                ...
                if (confirmed) {
                    window.location.href = '<?php echo e(route("tenant.catalog", $store->slug)); ?>';
                }
                return;
            }
...
async function loadOrderSummary() {
    try {
        const response = await fetch('<?php echo e(route("tenant.cart.get", $store->slug)); ?>');
        ...
        if (data.success) {
            ...
            if (cartItems.length > 0) {
                ...
                window.location.href = '<?php echo e(route("tenant.cart.index", $store->slug)); ?>';
            }
        } else {
            // Redirigir si no hay productos
            window.location.href = '<?php echo e(route("tenant.cart.index", $store->slug)); ?>';
...
async function calculateShippingCost(department, city) {
    ...
    try {
        const response = await fetch('<?php echo e(route("tenant.debug.shipping-cost", $store->slug)); ?>', {
            method: 'POST',
            headers: {
...
async function applyCoupon() {
    ...
    try {
        const response = await fetch('<?php echo e(route("tenant.cart.apply-coupon", $store->slug)); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
async function loadDepartmentsAndCities() {
    try {
        const response = await fetch('<?php echo e(route("tenant.checkout.shipping-departments", $store->slug)); ?>');
        const data = await response.json();
...
<?php echo $__env->make('frontend.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Liniu_Final\app\Features\Tenant/Views/checkout/create.blade.php ENDPATH**/ ?>