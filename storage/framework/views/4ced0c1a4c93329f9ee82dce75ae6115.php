...
<div class="px-4 py-6 space-y-6">
    ...
    <div id="cart-container" class="space-y-4">
        ...
        <div id="cart-actions" class="space-y-3 hidden">
            <a href="<?php echo e(route('tenant.checkout.create', $store->slug)); ?>" 
               id="checkout-btn" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createCartItem(item) {
        ...
        if (imageUrl) {
            ...
            imageElement.src = '<?php echo e(asset("assets/images/placeholder-product.svg")); ?>';
        }
    ...
    async function updateQuantity(itemKey, newQuantity) {
        ...
        try {
            const response = await fetch('<?php echo e(route("tenant.cart.update", $store->slug)); ?>', {
                method: 'PUT',
                headers: {
    ...
    async function removeItem(itemKey) {
        ...
        try {
            const response = await fetch('<?php echo e(route("tenant.cart.remove", $store->slug)); ?>', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    document.getElementById('clear-cart-btn').addEventListener('click', async function() {
        ...
        if (confirmed) {
            try {
                const response = await fetch('<?php echo e(route("tenant.cart.clear", $store->slug)); ?>', {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    }
...
<?php echo $__env->make('frontend.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Liniu_Final\app\Features\Tenant/Views/storefront/cart.blade.php ENDPATH**/ ?>