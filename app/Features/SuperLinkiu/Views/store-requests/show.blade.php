            ...
            <div x-show="showApproveModal" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('superlinkiu.store-requests.approve', $store) }}" method="POST">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Categoría de Negocio *</label>
                                <select name="business_category_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary-200 focus:border-primary-200">
                                    <option value="">Seleccionar categoría...</option>
                                    @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ $store->business_category_id == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                    @endforeach
                                </select>
