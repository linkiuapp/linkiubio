...
@section('title', 'Editar Alerta de Monitoreo')

@section('content')
<div class="max-w-6xl mx-auto space-y-6 mt-6" x-data="editAlert()" x-init="init()">
    {{-- SECTION: Header --}}
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-semibold text-gray-800">Editar Alerta</h1>
    ...
    <form action="{{ route('superlinkiu.monitoring.alerts.update', $alert->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <select name="type" 
                                    ...
                                    x-init="if (!alertType) alertType = '{{ $alert->type }}'"
                                ...
                                <option value="error_rate" {{ old('type', $alert->type) === 'error_rate' ? 'selected' : '' }}>Tasa de Errores</option>
                                <option value="traffic_spike" {{ old('type', $alert->type) === 'traffic_spike' ? 'selected' : '' }}>Pico de Tráfico</option>
                                <option value="slow_response" {{ old('type', $alert->type) === 'slow_response' ? 'selected' : '' }}>Respuesta Lenta</option>
                                <option value="high_memory" {{ old('type', $alert->type) === 'high_memory' ? 'selected' : '' }}>Alto Uso de Memoria</option>
                                <option value="custom" {{ old('type', $alert->type) === 'custom' ? 'selected' : '' }}>Personalizado</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                ...
                <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        @php
                            // Obtener condiciones guardadas directamente del modelo
                            $conditions = is_array($alert->conditions) ? $alert->conditions : (json_decode($alert->conditions, true) ?? []);
                            
                            // Si hay valores de old (errores de validación), sobrescribir solo esos campos específicos
                            if (old('conditions')) {
                                foreach (old('conditions') as $key => $value) {
                                    if ($value !== null) {
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('conditions.threshold', $conditions['threshold'] ?? 10) }}"
                                           min="1"
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('conditions.minutes', $conditions['minutes'] ?? 5) }}"
                                           min="1"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                </div>
                                <div>
                                    ...
                                    <select name="conditions[level]" 
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                                        <option value="ERROR" {{ old('conditions.level', $conditions['level'] ?? 'ERROR') === 'ERROR' ? 'selected' : '' }}>ERROR</option>
                                        <option value="WARNING" {{ old('conditions.level', $conditions['level'] ?? 'ERROR') === 'WARNING' ? 'selected' : '' }}>WARNING</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mt-4">
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('conditions.route_pattern', isset($conditions['route_pattern']) && $conditions['route_pattern'] !== null ? $conditions['route_pattern'] : '') }}"
                        ...
                        <div x-show="alertType === 'traffic_spike'">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" 
                                           name="conditions[threshold]" 
                                           value="{{ old('conditions.threshold', $conditions['threshold'] ?? 1000) }}"
                                           min="1"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                </div>
                                <div>
                                    ...
                                    <input type="number" 
                                           name="conditions[minutes]" 
                                           value="{{ old('conditions.minutes', $conditions['minutes'] ?? 1) }}"
                                           min="1"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('conditions.threshold_ms', $conditions['threshold_ms'] ?? 3000) }}"
                                           min="100"
                                           step="100"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                           name="conditions[minutes]" 
                                           value="{{ old('conditions.minutes', $conditions['minutes'] ?? 10) }}"
                                           min="1"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('conditions.threshold_mb', $conditions['threshold_mb'] ?? 512) }}"
                                           min="1"
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                           required>
                                </div>
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <select name="channel" 
                                    ...
                                    required>
                                <option value="email" {{ old('channel', $alert->channel) === 'email' ? 'selected' : '' }}>Email</option>
                                <option value="whatsapp" {{ old('channel', $alert->channel) === 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                                <option value="in_app" {{ old('channel', $alert->channel) === 'in_app' ? 'selected' : '' }}>In-App</option>
                            </select>
                            @error('channel')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   ...
                                   value="{{ old('cooldown_minutes', $alert->cooldown_minutes) }}"
                                   min="1"
                                   max="1440"
...
function editAlert() {
    return {
        alertType: '{{ old("type", $alert->type) }}',
        init() {
            // Asegurar que el tipo se inicialice correctamente
            if (!this.alertType) {
            ...
            // Ejecutar updateConditions al inicializar
            setTimeout(() => {
        ...
        updateConditions() {
            // Remover el atributo name de todos los inputs de condiciones
            const allConditionInputs = document.querySelectorAll('input[name^="conditions["], select[name^="conditions["]');
            allConditionInputs.forEach(input => {
                if (!input.hasAttribute('data-name')) {
                    input.setAttribute('data-name', input.getAttribute('name'));
