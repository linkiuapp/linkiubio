...
<div class="max-w-2xl mx-auto space-y-6 mt-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-lg font-semibold text-gray-800">Editar Etiqueta</h1>
    </div>

    <form action="{{ route('superlinkiu.tutorial-tags.update', $tutorialTag) }}" method="POST" class="bg-white rounded-lg border border-gray-200 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" 
                       name="name" 
                       value="{{ old('name', $tutorialTag->name) }}"
                       required
            ...
            <div>
                ...
                <input type="text" 
                       name="slug" 
                       value="{{ old('slug', $tutorialTag->slug) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none">
            </div>
        </div>
