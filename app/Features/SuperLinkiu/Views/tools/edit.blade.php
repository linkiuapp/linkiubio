...
@section('title', 'Editar Herramienta - ' . $tool->name)
...
<div class="max-w-6xl mx-auto space-y-6 mt-6" x-data="toolForm()">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('superlinkiu.linkiu-tools.show', $tool) }}" class="inline-flex items-center justify-center">
                <i data-lucide="arrow-left" class="w-5 h-5 text-gray-600 hover:text-gray-800"></i>
            </a>
            <div>
                <h1 class="text-lg font-semibold text-gray-800">Editar Herramienta</h1>
                <p class="text-sm text-gray-600 mt-1">{{ $tool->name }}</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('superlinkiu.linkiu-tools.update', $tool) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              x-model="description"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $tool->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="usage_in_linkiu" name="usage_in_linkiu" rows="4"
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('usage_in_linkiu', $tool->usage_in_linkiu) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url', $tool->url) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="dashboard_url" class="block text-sm font-medium text-gray-800 mb-2">URL del Panel</label>
                        <input type="url" id="dashboard_url" name="dashboard_url" value="{{ old('dashboard_url', $tool->dashboard_url) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="api_docs_url" class="block text-sm font-medium text-gray-800 mb-2">URL de Documentación API</label>
                        <input type="url" id="api_docs_url" name="api_docs_url" value="{{ old('api_docs_url', $tool->api_docs_url) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="billing_type" name="billing_type" 
                                ...
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required>
                            <option value="free" {{ old('billing_type', $tool->billing_type) === 'free' ? 'selected' : '' }}>Gratis</option>
                            <option value="monthly" {{ old('billing_type', $tool->billing_type) === 'monthly' ? 'selected' : '' }}>Mensual</option>
                            <option value="yearly" {{ old('billing_type', $tool->billing_type) === 'yearly' ? 'selected' : '' }}>Anual</option>
                            <option value="pay_per_use" {{ old('billing_type', $tool->billing_type) === 'pay_per_use' ? 'selected' : '' }}>Por uso</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="currency" name="currency" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="USD" {{ old('currency', $tool->currency) === 'USD' ? 'selected' : '' }}>USD</option>
                            <option value="COP" {{ old('currency', $tool->currency) === 'COP' ? 'selected' : '' }}>COP</option>
                            <option value="EUR" {{ old('currency', $tool->currency) === 'EUR' ? 'selected' : '' }}>EUR</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $tool->start_date?->format('Y-m-d')) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="last_renewal_date" class="block text-sm font-medium text-gray-800 mb-2">Última Renovación</label>
                        <input type="date" id="last_renewal_date" name="last_renewal_date" value="{{ old('last_renewal_date', $tool->last_renewal_date?->format('Y-m-d')) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="next_renewal_date" class="block text-sm font-medium text-gray-800 mb-2">Próxima Renovación</label>
                        <input type="date" id="next_renewal_date" name="next_renewal_date" value="{{ old('next_renewal_date', $tool->next_renewal_date?->format('Y-m-d')) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-base font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    Credenciales (Dejar vacío para mantener la actual, se encriptará si se modifica)
                </h2>
            </div>
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="username" name="username" value="{{ old('username', $tool->username) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        ...
                        @if($decryptedData['password'])
                            <p class="mt-1 text-xs text-gray-600">Actual: <span class="font-mono">{{ Str::limit($decryptedData['password'], 20) }}</span></p>
                        @endif
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        @if($decryptedData['api_key'])
                            <p class="mt-1 text-xs text-gray-600">Actual: <span class="font-mono">{{ Str::limit($decryptedData['api_key'], 20) }}</span></p>
                        @endif
                    </div>
                    <div>
                        <label for="api_secret" class="block text-sm font-medium text-gray-800 mb-2">API Secret</label>
                        <div class="relative">
                            <input type="password" id="api_secret" name="api_secret" value=""
                        ...
                        @if($decryptedData['api_secret'])
                            <p class="mt-1 text-xs text-gray-600">Actual: <span class="font-mono">{{ Str::limit($decryptedData['api_secret'], 20) }}</span></p>
                        @endif
                    </div>
                </div>
                <div>
                    <label for="account_id" class="block text-sm font-medium text-gray-800 mb-2">ID de Cuenta</label>
                    <input type="text" id="account_id" name="account_id" value="{{ old('account_id', $tool->account_id) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
        </div>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="status" name="status" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required>
                            <option value="active" {{ old('status', $tool->status) === 'active' ? 'selected' : '' }}>Activa</option>
                            <option value="inactive" {{ old('status', $tool->status) === 'inactive' ? 'selected' : '' }}>Inactiva</option>
                            <option value="deprecated" {{ old('status', $tool->status) === 'deprecated' ? 'selected' : '' }}>Deprecada</option>
                        </select>
                    </div>
                    <div>
                        <label for="responsible_team" class="block text-sm font-medium text-gray-800 mb-2">Equipo Responsable</label>
                        <input type="text" id="responsible_team" name="responsible_team" value="{{ old('responsible_team', $tool->responsible_team) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div>
                    ...
                    <input type="text" id="notification_phone" name="notification_phone" value="{{ old('notification_phone', $tool->notification_phone) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="is_critical" name="is_critical" value="1"
                           {{ old('is_critical', $tool->is_critical) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $tool->notes) }}</textarea>
                </div>
            </div>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('toolForm', () => ({
        ...
        usageInLinkiu: `{{ $tool->usage_in_linkiu ?? '' }}`,
        billingType: '{{ $tool->billing_type }}',
        showPassword: false,
        showApiKey: false,
        showApiSecret: false,
