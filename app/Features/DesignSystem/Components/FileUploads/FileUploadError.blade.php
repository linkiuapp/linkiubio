{{--
File Upload Error - Subida de archivos con validación de tamaño
Uso: Permite subir archivos con validación de tamaño máximo y mensajes de error
Cuándo usar: Cuando necesites validar el tamaño de archivo antes de subirlo
Cuándo NO usar: Cuando no necesites validación (usa FileUploadBasic)
Ejemplo: <x-file-upload-error uploadUrl="/upload" maxFileSize="1" />
...
@php
    $uniqueId = $uniqueId ?? 'file-upload-error-' . uniqid();
    $config = json_encode([
        'url' => $uploadUrl,
            ...
            <div class="mb-1 flex justify-between items-center">
                <div class="flex items-center gap-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-red-500" style="display: none;" data-hs-file-upload-file-error="">El archivo excede el límite de tamaño.</p>
                    </div>
                </div>
                <div class="flex items-center gap-x-2">
                    <span class="hs-tooltip [--placement:top] inline-block" style="display: none;" data-hs-file-upload-file-error="">
                        <span class="hs-tooltip-toggle text-red-500 hover:text-red-800 focus:outline-hidden focus:text-red-800">
                            <i data-lucide="alert-circle" class="shrink-0 size-4"></i>
                            <span class="hs-tooltip-content max-w-25 hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible z-10 py-1 px-2 bg-gray-900 text-xs font-medium text-white rounded-md shadow-2xs" role="tooltip">
                                Por favor, intenta subir un archivo menor a {{ $maxFileSize }}MB.
                            </span>
                        </span>
                    </span>
    ...
    <div class="cursor-pointer p-12 flex justify-center bg-white border border-dashed border-gray-300 rounded-xl" data-hs-file-upload-trigger="">
        <div class="text-center">
            ...
            <p class="mt-1 text-xs text-gray-400">
                Selecciona un archivo de hasta {{ $maxFileSize }}MB.
            </p>
        </div>
    </div>
...
(function() {
    ...
    // Esperar a que Preline UI y Dropzone estén disponibles
    function initFileUploadError() {
        if (typeof HSFileUpload === 'undefined') {
            setTimeout(initFileUploadError, 100);
            return;
        }
        
        try {
            const instance = HSFileUpload.getInstance('#{{ $uniqueId }}', true);
            ...
            dropzone.on('error', (file, response) => {
                if (file.size > instance.element.concatOptions.maxFilesize * 1024 * 1024) {
                    const successEls = document.querySelectorAll('#{{ $uniqueId }} [data-hs-file-upload-file-success]');
                    const errorEls = document.querySelectorAll('#{{ $uniqueId }} [data-hs-file-upload-file-error]');
            ...
            console.warn('Error al inicializar File Upload Error:', error);
        }
    }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(initFileUploadError, 500);
        });
    } else {
        setTimeout(initFileUploadError, 500);
    }
