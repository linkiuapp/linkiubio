{{--
AnnouncementCarouselStatic - Carousel de anuncios/banners (versión optimizada)
Uso: Mostrar banners desde datos backend (sin fetch asíncrono)
Cuándo usar: Dashboards donde los banners ya están cacheados en backend
Cuándo NO usar: Cuando los banners deben cargarse dinámicamente vía API
Ejemplo: <x-announcement-carousel-static :banners="$banners" />
--}}
...
<div class="relative w-full {{ $height }} rounded-xl overflow-hidden shadow-sm" 
     x-data="announcementCarouselStatic({{ $banners->toJson() }})" 
     x-init="init()"
     {{ $attributes }}>
    {{-- Carousel Container --}}
...
document.addEventListener('alpine:init', () => {
    Alpine.data('announcementCarouselStatic', (bannersData) => ({
        banners: bannersData || [],
        currentSlide: 0,
