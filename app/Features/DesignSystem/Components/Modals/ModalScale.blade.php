{{--
Modal Scale - Modal con animación de escala
Uso: Modal con efecto de animación de escala clásico
Cuándo usar: Cuando quieras una animación más llamativa al abrir el modal
Cuándo NO usar: Cuando prefieras una animación más sutil
Ejemplo: <x-modal-scale modalId="my-modal" title="Título" content="Contenido" />
--}}
...
@php
    $uniqueId = $modalId ?? 'modal-scale-' . uniqid();
@endphp
...
<div x-data="{ open: false }" x-on:keydown.escape.window="open = false">
    ...
    >
        <div 
            class="sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-56px)] flex items-center pointer-events-none"
