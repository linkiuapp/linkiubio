{{--
Botón Bloque - Botón que ocupa el ancho completo de su contenedor
Uso: Formularios principales, acciones primarias en móviles, CTAs principales
Cuándo usar: Formularios de registro/login, checkout, acciones principales en pantallas pequeñas
Cuándo NO usar: Acciones secundarias, botones en toolbars, múltiples acciones en línea
Ejemplo: <x-button-block type="solid" color="info" text="Crear cuenta" />
--}}
...
@php
    // Size classes for full width buttons
    $sizeClasses = [
    ...
    $typeColorClasses = [
        ...
        'link' => [
            'dark' => 'text-gray-800 hover:text-blue-600 focus:text-blue-600',
            'secondary' => 'text-gray-500 hover:text-blue-600 focus:text-blue-600',
            'success' => 'text-green-600 hover:text-green-800 focus:text-green-800',
            'info' => 'text-blue-600 hover:text-blue-800 focus:text-blue-800',
            'error' => 'text-red-600 hover:text-red-800 focus:text-red-800',
            'warning' => 'text-yellow-600 hover:text-yellow-800 focus:text-yellow-800',
            'light' => 'text-white hover:text-white/80 focus:text-white/80',
        ],
    ];
    
    // Base classes with full width
    $baseClasses = 'w-full inline-flex justify-center items-center gap-x-2 font-medium rounded-lg focus:outline-none transition-colors disabled:opacity-50 disabled:pointer-events-none';
    ...
    // Font weight for link type
    $fontClass = $type === 'link' ? 'font-semibold' : 'font-medium';
    
    // Final classes
    $classes = trim("{$baseClasses} {$sizeClass} {$typeColorClass} {$borderClass} {$fontClass}");
@endphp
