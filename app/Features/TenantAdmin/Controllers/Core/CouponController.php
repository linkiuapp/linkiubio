...
class CouponController extends Controller
{
    ...
    public function destroy(Request $request, $storeSlug, Coupon $coupon)
    {
        ...
        return redirect()
            ->route('tenant.admin.coupons.index', $store->slug)
            ->with('coupon_deleted', true);
    }
    
    /**
     * Toggle coupon status.
     */
    public function toggleStatus(Request $request, $storeSlug, Coupon $coupon)
    {
        $store = $request->route('store');
        ...
        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => "El cupón se ha {$status} correctamente.",
                'is_active' => $coupon->is_active,
            ]);
        }

        return redirect()
            ->route('tenant.admin.coupons.show', ['store' => $store->slug, 'coupon' => $coupon])
            ->with('coupon_status_updated', "El cupón se ha {$status} correctamente.");
    }
