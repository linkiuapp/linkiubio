<x-tenant-admin-layout :store="$store">
    ...
    <div
        ...
        x-data="(() => ({
            isScheduled: {{ json_encode((bool) old('is_scheduled', $slider->is_scheduled)) }},
            isPermanent: {{ json_encode((bool) old('is_permanent', $slider->is_permanent)) }},
            urlType: {{ json_encode(old('url_type', $slider->url_type ?? 'none')) }},
            urlValue: {{ json_encode(old('url', $slider->url)) }},
            selectedInternalLabel: {{ json_encode(old('url_type', $slider->url_type ?? 'none') === 'internal' ? old('url', $slider->url) : '') }},
            suggestions: [],
    ...
    >
        ...
        <!-- Alerta informativa -->
        <div class="mb-6">
            <x-alert-soft type="info" message="Estás usando {{ $currentCount }} de {{ $maxSliders }} sliders disponibles en tu plan {{ $store->plan->name }}." />
        </div>

        <!-- Formulario dentro de Card -->
        <x-card-base size="lg" shadow="sm">
            <form action="{{ route('tenant.admin.sliders.update', [$store->slug, $slider->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    @if($slider->image_path)
                        ...
                        <img src="{{ Storage::disk('public')->url($slider->image_path) }}" alt="Imagen actual del slider" class="h-40 rounded-lg mt-2 border border-gray-200 object-cover">
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-6 space-y-3">
                        ...
                        <x-radio-basic 
                            radio-name="url_type"
                            label="Sin enlace"
                            :checked="old('url_type', $slider->url_type) == 'none'"
                        ...
                        <x-radio-basic 
                            ...
                            :checked="old('url_type', $slider->url_type) == 'internal'"
                        ...
                        <x-radio-basic 
                            ...
                            :checked="old('url_type', $slider->url_type) == 'external'"
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            <x-switch-basic 
                                ...
                                :checked="old('is_scheduled', $slider->is_scheduled)"
                    ...
                    <div x-show="isScheduled" x-cloak style="display: none;" class="space-y-6">
                        <div class="flex items-center gap-3">
                            ...
                            <x-switch-basic 
                                ...
                                :checked="old('is_permanent', $slider->is_permanent)"
                        ...
                        <div x-show="!isPermanent" x-cloak style="display: none;" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-with-label 
                                    ...
                                    name="start_date"
                                    type="date"
                                    :value="old('start_date', optional($slider->start_date)->toDateString())"
                                />
                            </div>
                            <div>
                                <x-input-with-label 
                                    ...
                                    name="end_date"
                                    type="date"
                                    :value="old('end_date', optional($slider->end_date)->toDateString())"
                                />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-with-label 
                                    ...
                                    name="start_time"
                                    type="time"
                                    :value="old('start_time', optional($slider->start_time)->format('H:i'))"
                                />
                            </div>
                            <div>
                                <x-input-with-label 
                                    ...
                                    name="end_time"
                                    type="time"
                                    :value="old('end_time', optional($slider->end_time)->format('H:i'))"
                                />
                            </div>
                        </div>
                                ...
                                @foreach(['monday' => 'L', 'tuesday' => 'M', 'wednesday' => 'X', 'thursday' => 'J', 'friday' => 'V', 'saturday' => 'S', 'sunday' => 'D'] as $day => $label)
                                    <label class="flex items-center justify-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                                        <input type="checkbox" 
                                               ...
                                               value="1" 
                                               {{ old("scheduled_days.{$day}", $slider->scheduled_days[$day] ?? false) ? 'checked' : '' }}
                                               class="sr-only peer">
                ...
                <div class="flex justify-between items-center pt-6 border-t border-gray-200">
                    ...
                    <x-button-icon type="solid" color="dark" icon="save" text="Guardar Cambios" html-type="submit" />
                </div>
            </form>
        </x-card-base>
    </div>
