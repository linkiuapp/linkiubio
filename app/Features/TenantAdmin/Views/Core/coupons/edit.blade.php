<x-tenant-admin-layout :store="$store">
    ...
    <div class="max-w-4xl mx-auto space-y-6" x-data="couponEditForm()" x-init="init()">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $coupon->name }} — {{ $coupon->code }}</p>
                </div>
            </div>
            <a href="{{ route('tenant.admin.coupons.show', ['store' => $store->slug, 'coupon' => $coupon]) }}">
                <x-button-icon type="outline" color="secondary" size="sm" icon="eye" text="Ver detalles" />
            </a>
        </div>
        ...
        @if($coupon->current_uses > 0)
            ...
            <x-alert-soft
                type="warning"
                :message="'Este cupón ya se utilizó ' . $coupon->current_uses . ' vez' . ($coupon->current_uses === 1 ? '' : 'es') . '. Los cambios no afectarán órdenes anteriores.'"
            />
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div class="p-6 space-y-8">
                    ...
                    <section class="space-y-4">
                        ...
                        >{{ old('description', $coupon->description) }}</x-textarea-with-label>
                    </section>
                        ...
                        <div class="grid gap-4 md:grid-cols-3">
                            ...
                            <div>
                                <x-select-with-label
                                    ...
                                    name="discount_type"
                                    :selected="old('discount_type', $coupon->discount_type)"
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div>
                                <x-input-with-label
                                    ...
                                    container-class="w-full"
                                    placeholder="Ej: 100"
                                    min="{{ $coupon->current_uses }}"
                                    value="{{ old('max_uses', $coupon->max_uses) }}"
                                />
                                @if($coupon->current_uses > 0)
                                    <p class="mt-1 text-xs text-gray-500">El cupón registra {{ $coupon->current_uses }} uso{{ $coupon->current_uses === 1 ? '' : 's' }}.</p>
                                @endif
                                @error('max_uses')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                        ...
                                        @foreach($days as $index => $day)
                                            <label class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm hover:bg-gray-50">
                                                <input type="checkbox"
                                                    ...
                                                    name="days_of_week[]"
                                                    value="{{ $index }}"
                                                    @checked(is_array($selectedDays) && in_array($index, $selectedDays))
                                                >
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <x-input-with-label
                                            ...
                                            id="start-time"
                                            container-class="w-full"
                                            value="{{ old('start_time', optional($coupon->start_time)->format('H:i')) }}"
                                        />
                                        @error('start_time')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        <x-input-with-label
                                            ...
                                            id="end-time"
                                            container-class="w-full"
                                            value="{{ old('end_time', optional($coupon->end_time)->format('H:i')) }}"
                                        />
                                        @error('end_time')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                ...
                <div class="border-t border-gray-200 p-6 space-y-6">
                    ...
                    <section class="flex justify-between gap-4 md:grid-cols-3">
                        <div class="flex items-center gap-3">
                            ...
                            <x-switch-basic switch-id="is-active" switch-name="is_active" value="1" :checked="(bool) old('is_active', $coupon->is_active)" />
                            <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <x-switch-basic switch-id="is-public" switch-name="is_public" value="1" :checked="(bool) old('is_public', $coupon->is_public)" />
                            <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <x-switch-basic switch-id="is-automatic" switch-name="is_automatic" value="1" :checked="(bool) old('is_automatic', $coupon->is_automatic)" />
                            <div>
                    ...
                    <section class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div class="flex gap-3 md:order-2">
                            <x-button-icon type="solid" color="dark" icon="loader" text="Actualizar cupón" html-type="submit" />
    ...
    <script>
        function couponEditForm() {
            return {
                form: {
                    name: @js(old('name', $coupon->name)),
                    code: @js(old('code', $coupon->code)),
                    description: @js(old('description', $coupon->description)),
                    type: @js(old('type', $coupon->type)),
                    discountType: @js(old('discount_type', $coupon->discount_type)),
                    discountValue: @js(old('discount_value', $coupon->discount_value)),
                },
                showTimeRestrictions: {{ (old('days_of_week', $coupon->days_of_week ?? []) || old('start_time', $coupon->start_time) || old('end_time', $coupon->end_time)) ? 'true' : 'false' }},
