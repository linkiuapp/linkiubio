<x-tenant-admin-layout :store="$store">
    ...
    @section('content')
        ...
        {{-- SECTION: Alertas Proactivas de KiuBot --}}
        @if($store->plan && $store->plan->kiubot_enabled && !empty($insights['alerts']))
        <div class="mb-6" x-data="{ showAllAlerts: false }">
            <div class="flex items-center justify-between mb-3">
                ...
                @if(count($insights['alerts']) > 2)
                <button 
                    @click="showAllAlerts = !showAllAlerts"
                    class="text-xs text-indigo-600 hover:text-indigo-800 font-medium"
                >
                    <span x-text="showAllAlerts ? 'Ver menos' : 'Ver todas ({{ count($insights['alerts']) }})'"></span>
                </button>
                @endif
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @foreach($insights['alerts'] as $index => $alert)
                <div 
                    class="rounded-lg p-4 border transition-all hover:shadow-md
                        @if($alert['type'] === 'warning') bg-amber-50 border-amber-200
                        @elseif($alert['type'] === 'danger') bg-red-50 border-red-200
                        @else bg-blue-50 border-blue-200
                        @endif"
                    x-show="showAllAlerts || {{ $index }} < 2"
                    x-transition
                >
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center
                            @if($alert['type'] === 'warning') bg-amber-100
                            @elseif($alert['type'] === 'danger') bg-red-100
                            @else bg-blue-100
                            @endif">
                            @if($alert['icon'] === 'package-x')
                                <i data-lucide="package-x" class="w-5 h-5 text-amber-600"></i>
                            @elseif($alert['icon'] === 'alert-triangle')
                                <i data-lucide="alert-triangle" class="w-5 h-5 text-amber-600"></i>
                            @elseif($alert['icon'] === 'clock')
                                <i data-lucide="clock" class="w-5 h-5 text-blue-600"></i>
                            @elseif($alert['icon'] === 'trending-down')
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if(isset($alert['action']))
                                ...
                                <a href="{{ route($alert['action']['route'], ['store' => $store->slug]) }}" 
                                   class="inline-flex items-center gap-1 mt-2 text-xs font-medium text-indigo-600 hover:text-indigo-800">
                                    {{ $alert['action']['label'] }}
                                    <i data-lucide="chevron-right" class="w-3 h-3"></i>
                                </a>
                                @elseif(isset($alert['action']['type']) && $alert['action']['type'] === 'chat')
                                <button 
                                    onclick="window.sendKiuBotQuestion && window.sendKiuBotQuestion('{{ $alert['action']['prompt'] }}')"
                                    class="inline-flex items-center gap-1 mt-2 text-xs font-medium text-indigo-600 hover:text-indigo-800">
                                    {{ $alert['action']['label'] }}
                                    <i data-lucide="chevron-right" class="w-3 h-3"></i>
                                </button>
        ...
        {{-- SECTION: Insights Rápidos --}}
        @if($store->plan && $store->plan->kiubot_enabled && !empty($insights['performance']))
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-5 text-white shadow-lg">
                ...
                <div class="text-3xl font-bold">${{ number_format($insights['performance']['sales']['this_week'] ?? 0, 0, ',', '.') }}</div>
                <p class="text-xs text-indigo-200 mt-1">vs ${{ number_format($insights['performance']['sales']['last_week'] ?? 0, 0, ',', '.') }} semana pasada</p>
            </div>

            {{-- Widget de Producto Estrella --}}
            @if(!empty($insights['performance']['top_product']))
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                ...
                <p class="font-bold text-gray-900 truncate">{{ $insights['performance']['top_product']['name'] }}</p>
                <div class="flex items-center gap-4 mt-2">
                    <span class="text-sm text-gray-600">
                        <span class="font-semibold text-indigo-600">{{ $insights['performance']['top_product']['units_sold'] }}</span> vendidos
                    </span>
                    <span class="text-sm text-gray-600">
                        ${{ number_format($insights['performance']['top_product']['revenue'] ?? 0, 0, ',', '.') }}
                    </span>
                </div>
            </div>
            @else
            ...
            {{-- Widget de Meta del Mes --}}
            @if(!empty($insights['goals']))
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-sm font-medium text-gray-600">Meta del mes</span>
                    <span class="text-xs text-gray-500">{{ $insights['goals']['current_month']['days_remaining'] ?? 0 }} días restantes</span>
                </div>
                <div class="mb-2">
                    <div class="flex justify-between text-sm mb-1">
                        <span class="font-bold text-gray-900">{{ $insights['goals']['goal']['progress_percent'] ?? 0 }}%</span>
                        <span class="text-gray-500">${{ number_format($insights['goals']['goal']['amount'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-500"
                             style="width: {{ min(100, $insights['goals']['goal']['progress_percent'] ?? 0) }}%"></div>
                    </div>
                </div>
                @if(($insights['goals']['goal']['orders_needed'] ?? 0) > 0)
                <p class="text-xs text-gray-500">Necesitas {{ $insights['goals']['goal']['orders_needed'] }} pedidos más para alcanzar tu meta</p>
                @else
        ...
        {{-- SECTION: Sugerencias Estacionales --}}
        @if($store->plan && $store->plan->kiubot_enabled && !empty($insights['seasonal']))
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="font-bold text-gray-900">{{ $event['event'] }}</span>
                        <span class="px-2 py-0.5 bg-rose-100 text-rose-700 text-xs rounded-full">
                            {{ $event['days_until'] }} días
                        </span>
                    </div>
                    <p class="text-sm text-gray-600 mb-2">{{ $event['suggestion'] }}</p>
                    <div class="flex items-center gap-3">
                        <a href="{{ route('tenant.admin.coupons.create', ['store' => $store->slug]) }}" 
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-gray-50 text-gray-800 text-xs font-medium rounded-lg border border-gray-200 transition-colors">
                            ...
                            Crear cupón "{{ $event['suggested_coupon'] }}"
                        </a>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6" x-data="assistantChat()" data-tour="kiubot-section">
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images-ui/emoji_kiubot_linkiu.svg') }}" alt="KiuBot" class="w-12 h-12 flex-shrink-0">
                    <div>
            ...
            <div class="h-[450px] overflow-y-auto mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200" id="chat-messages" x-ref="messagesContainer">
                ...
                <div class="chat chat-start mb-4">
                    <div class="chat-image avatar">
                        <img src="{{ asset('images-ui/emoji_kiubot_linkiu.svg') }}" alt="KiuBot" class="w-10 h-10">
                    </div>
            ...
            <div class="mb-4">
                ...
                {{-- Sección: Análisis y sugerencias --}}
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('assistantChat', () => ({
                ...
                async sendMessage() {
                    ...
                    try {
                        const response = await fetch('{{ route("tenant.admin.dashboard.chat", ["store" => $store->slug]) }}', {
                            method: 'POST',
                            headers: {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const shouldShow = {{ $shouldShowTour ? 'true' : 'false' }};
            const resetTour = {{ $resetTour ? 'true' : 'false' }};
    ...
    @endpush
    {{-- End SECTION: Tour de Bienvenida --}}
