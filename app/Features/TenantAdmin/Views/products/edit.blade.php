<x-tenant-admin-layout :store="$store">
            ...
            <div class="bg-accent-50 rounded-lg p-0 overflow-hidden" id="variables-section" style="display: {{ $product->type === 'variable' ? 'block' : 'none' }};">
                <div class="border-b border-accent-100 bg-accent-50 py-4 px-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-black-500">Variables del Producto</h3>
                            <p class="text-sm text-black-300 mt-1">Asigna variables personalizables a este producto</p>
                        </div>
                        @if($variables->count() == 0)
                ...
                <div class="p-6">
                    @if($variables->count() > 0)
                        <div class="space-y-4">
                            @foreach($variables as $variable)
                                ...
                                @endphp
                                <div class="border border-accent-200 rounded-lg p-4 hover:border-primary-200 transition-colors">
                                        ...
                                        <div class="flex-1">
                                            <label for="variable_{{ $variable->id }}" class="cursor-pointer">
                                                <div class="flex items-center gap-2 mb-1">
                                                    <h4 class="text-sm font-medium text-black-400">{{ $variable->name }}</h4>
                                                    <span class="px-2 py-0.5 bg-info-100 text-info-300 rounded text-xs">{{ $variable->type_name }}</span>
                                                    @if($isAssigned)
                                                        <span class="px-2 py-0.5 bg-success-100 text-success-300 rounded text-xs">Asignada</span>
                                                    @endif
                                                </div>
                                            ...
                                            <div class="mt-3 space-y-2 variable-options" id="options_{{ $variable->id }}" style="display: {{ $isAssigned ? 'block' : 'none' }};">
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    <!-- Toggle Requerido -->
                                                    <div class="flex items-center gap-2">
                                                        <input type="checkbox" 
                                                               id="required_{{ $variable->id }}"
                                                               name="variables[{{ $variable->id }}][is_required]"
                                                               value="1"
                                                               {{ $isAssigned && $assignment->is_required ? 'checked' : ($variable->is_required_default ? 'checked' : '') }}
                                                               class="w-4 h-4 text-error-300 border-accent-300 rounded focus:ring-error-200">
                                                        <label for="required_{{ $variable->id }}" class="text-sm text-black-400 cursor-pointer">
                                                            Campo requerido
                                                        </label>
                                                    </div>
                                                    ...
                                                    <div>
                                                        <label class="text-xs text-black-300 mb-1 block">Orden</label>
                                                        <input type="number" 
                                                               name="variables[{{ $variable->id }}][display_order]"
                                                               value="{{ $isAssigned ? $assignment->display_order : $loop->iteration }}"
                                                               min="1"
                                                               class="w-20 px-2 py-1 text-sm border border-accent-200 rounded focus:outline-none focus:ring-1 focus:ring-primary-200">
                                                    </div>
                                                </div>
                                                
                                                <!-- Etiqueta personalizada -->
                                                <div>
                                                    <label class="text-xs text-black-300 mb-1 block">Etiqueta personalizada (opcional)</label>
                                                    <input type="text" 
                                                           ...
                                                           class="w-full px-3 py-2 text-sm border border-accent-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary-200">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    @else
                        <div class="text-center py-8 text-black-300">
                            <x-solar-settings-outline class="w-12 h-12 mx-auto mb-3 text-black-200" />
    ...
    <script>
        ...
        typeSelect.addEventListener('change', toggleVariablesSection);
        
        // Mostrar/ocultar opciones de cada variable al seleccionarla
        document.querySelectorAll('input[name^="variables"][name$="[enabled]"]').forEach(checkbox => {
            const variableId = checkbox.id.replace('variable_', '');
