...
<div class="bg-white">
    ...
    <div class="mb-6">
        <h3 class="text-body-large font-bold text-black-500 mb-3">Facturado a:</h3>
        <div class="bg-gray-50 p-4 rounded-lg">
            <p class="font-bold text-black-500">{{ $invoice->store->owner_name ?? $invoice->store->name }}</p>
            @if($invoice->store->email)
                <p class="text-caption text-black-400">{{ $invoice->store->email }}</p>
            @endif
            @if($invoice->store->phone)
                <p class="text-caption text-black-400">{{ $invoice->store->phone }}</p>
            @endif
            @if($invoice->store->address)
                <p class="text-caption text-black-400">{{ $invoice->store->address }}</p>
            @endif
        </div>
    </div>
    ...
    <div class="mb-6">
        <h3 class="text-body-large font-bold text-black-500 mb-3">Detalles del servicio:</h3>
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-caption font-bold text-black-500">Descripción</th>
                        <th class="px-4 py-3 text-left text-caption font-bold text-black-500">Plan</th>
                        <th class="px-4 py-3 text-left text-caption font-bold text-black-500">Período</th>
                        <th class="px-4 py-3 text-right text-caption font-bold text-black-500">Monto</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="border-t border-gray-200">
                        <td class="px-4 py-3 text-caption text-black-400">
                            Suscripción plan {{ $invoice->plan->name ?? 'N/A' }}
                            <br>
                            <span class="text-xs text-black-300">Servicio de tienda online</span>
                        </td>
                        <td class="px-4 py-3 text-caption text-black-400">{{ $invoice->plan->name ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-caption text-black-400">
                        ...
                        <td class="px-4 py-3 text-right text-caption font-bold text-black-500">
        ...
        <div class="w-64">
            <div class="bg-gray-50 p-4 rounded-lg">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-caption font-bold text-black-500">${{ number_format($invoice->amount, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between items-center mb-2">
                    <span class="text-caption text-black-400">IVA (0%):</span>
                    <span class="text-caption font-bold text-black-500">$0</span>
                </div>
                <div class="border-t border-gray-200 pt-2">
                    <div class="flex justify-between items-center">
                        <span class="text-caption font-bold text-black-500">Total:</span>
                        <span class="text-body-large font-bold text-primary-400">${{ number_format($invoice->amount, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mb-6">
        <div class="flex items-center">
            <span class="text-caption text-black-400 mr-2">Estado:</span>
            <span class="px-3 py-1 rounded-full text-caption font-bold 
    ...
    <!-- Footer -->
    <div class="text-center text-caption text-black-300 pt-4 border-t border-gray-200">
        @if($billingSettings->footer_text)
            <p class="mb-2">{{ $billingSettings->footer_text }}</p>
        @endif
        <p class="font-bold">¡Gracias por confiar en Linkiu.bio!</p>
    </div>
</div>
