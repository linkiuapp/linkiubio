...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="bg-accent-50 rounded-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-lg font-semibold text-black-500 mb-2">Editar Tipo de Habitación</h2>
                <p class="text-sm text-black-300">{{ $roomType->name }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('tenant.admin.hotel.room-types.update', ['store' => $store->slug, 'roomType' => $roomType->id]) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">{{ old('description', $roomType->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="max_occupancy" value="{{ old('max_occupancy', $roomType->max_occupancy) }}" min="1" max="50" required
                               class="w-full px-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">
                        @error('max_occupancy')
                            <p class="mt-1 text-sm text-error-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="base_occupancy" value="{{ old('base_occupancy', $roomType->base_occupancy) }}" min="1" max="50" required
                               class="w-full px-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="max_adults" value="{{ old('max_adults', $roomType->max_adults) }}" min="1"
                               class="w-full px-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-black-400 mb-2">Máximo Niños (opcional)</label>
                        <input type="number" name="max_children" value="{{ old('max_children', $roomType->max_children) }}" min="0"
                               class="w-full px-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="base_price_per_night" value="{{ old('base_price_per_night', $roomType->base_price_per_night) }}" step="0.01" min="0" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-black-300">$</span>
                            <input type="number" name="extra_person_price" value="{{ old('extra_person_price', $roomType->extra_person_price) }}" step="0.01" min="0"
                                   class="w-full pl-8 pr-4 py-2 border border-accent-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-200">
                        </div>
