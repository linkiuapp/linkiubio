...
# Código completo de show categoría sin componentes
                ...
                <div class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 flex-wrap mt-1">
                                {{-- Badge Soft sin componente --}}
                                <span class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium {{ $category->is_active ? 'bg-teal-100 text-teal-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    <div class="shrink-0">
                        <a href="{{ route('tenant.admin.categories.edit', [$store->slug, $category->id]) }}">
                            <button type="button" 
                                    class="inline-flex items-center gap-x-2 font-medium rounded-lg focus:outline-none transition-colors disabled:opacity-50 disabled:pointer-events-none py-2 px-3 body-small bg-gray-800 text-white hover:bg-gray-900 focus:bg-gray-900 border border-transparent">
                                <i data-lucide="edit" class="shrink-0 w-3 h-3"></i>
                                Editar
                            </button>
                        </a>
                    </div>
...
## Cambios realizados en Show

Se reemplazaron los siguientes componentes por HTML directo:

1. **`x-badge-soft`** → Span con clases de badge suave (bg-teal-100 text-teal-800 para success, bg-red-100 text-red-800 para error)
2. **`x-button-icon`** → Button HTML con icono Lucide y clases de botón solid dark tamaño sm

**Nota:** El componente `x-tenant-admin-layout` se mantiene ya que es el layout principal de la aplicación.
